...
- Généré le : 2026-09-19 23:59
- Branche : `main` — dernier commit : 4f3ae9f « Ajoute les écrans v1 : layouts, onboarding, génération, semaine, courses, garde-manger, profil, jour, recette, swap » (2026-09-19)
- Arbre de travail : 12 fichier(s) modifié(s) non commité(s)
- Code : core 19 fichier(s) · tests 134 cas dans 17 fichier(s) · données ≈ 66 recette(s), ≈ 150 ingrédient(s) · routes 10 · composants 16 · store 6
...
- **Ce qui n'existe pas encore** (v1 restante) : écrans `item/[ingredientId]` (changer le pack, « j'en ai déjà », retirer), `store-mode`, `pantry-add` (recherche + ajout manuel), `cook-with`, `report` (bilan) ; ajout d'article libre et bouton « Tout ajouter au garde-manger » (`commitPurchasesToPantry`) dans Courses ; test sur téléphone (Expo Go) jamais fait.
...
1. Écrans restants (SPEC § 1.3) : `item/[ingredientId]` (changer le pack, « j'en ai déjà », retirer), ajout d'article libre + bouton « Tout ajouter au garde-manger » (`commitPurchasesToPantry`) dans Courses, `pantry-add`, `cook-with`, `report`, `store-mode`.
2. Recalibrer le score d'équilibre nutritionnel (voir constat ci-dessus) ; mesurer le temps de génération sur téléphone (Expo Go) et réduire `anneal.iterations` si > 500 ms.
