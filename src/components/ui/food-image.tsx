...
export interface FoodImageProps {
  ...
  /** Clé directe de `FOOD_IMAGES` (illustration d'une famille de cuisine, par exemple). */
  readonly imageKey?: FoodImageKey;
  /** Côté du carré (pt). */
...
export function FoodImage({ ingredientId, recipeId, imageKey, size = 56, tile = 'soft', style }: FoodImageProps) {
  const theme = useTheme();
  const key = imageKey ?? foodImageKey(ingredientId, recipeId);
